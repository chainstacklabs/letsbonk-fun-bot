...
TOKEN_AMOUNT_TO_SELL = int(os.environ.get("TOKEN_AMOUNT", "1000000"))  # Amount of tokens to sell (in base units)
...
def calculate_minimum_amount_out_from_pool_state(
    ...
    try:
        ...
        print(f"Expected output: {expected_output:,} lamports ({expected_output / LAMPORTS_PER_SOL:.6f} SOL)")
        print(f"Minimum with {slippage_tolerance*100}% slippage: {minimum_with_slippage:,} lamports ({minimum_with_slippage / LAMPORTS_PER_SOL:.6f} SOL)")
...
async def sell_exact_in(
    ...
    try:
        ...
        print(f"Minimum amount out: {minimum_amount_out:,} lamports ({minimum_amount_out / LAMPORTS_PER_SOL:.6f} SOL)")
        ...
        accounts = [
            ...
            AccountMeta(pubkey=user_base_token, is_signer=False, is_writable=True),        # user_base_token (tokens being sold)
            AccountMeta(pubkey=user_quote_token, is_signer=False, is_writable=True),       # user_quote_token (WSOL received)
            AccountMeta(pubkey=base_vault, is_signer=False, is_writable=True),             # base_vault (receives tokens)
            AccountMeta(pubkey=quote_vault, is_signer=False, is_writable=True),            # quote_vault (sends WSOL)
...
async def main():
    ...
    try:
        ...
        async with AsyncClient(RPC_ENDPOINT) as client:
            ...
            user_base_token = get_associated_token_address(PAYER.pubkey(), TOKEN_MINT_ADDRESS)
            try:
                token_account_info = await client.get_token_account_balance(user_base_token)
                if token_account_info.value:
                    ...
                    if token_balance < TOKEN_AMOUNT_TO_SELL:
                        print(f"Insufficient token balance! You have {token_balance:,} tokens but want to sell {TOKEN_AMOUNT_TO_SELL:,}")
                        return
                else:
            ...
            tx_signature = await sell_exact_in(client, TOKEN_MINT_ADDRESS, TOKEN_AMOUNT_TO_SELL, SLIPPAGE_TOLERANCE)
