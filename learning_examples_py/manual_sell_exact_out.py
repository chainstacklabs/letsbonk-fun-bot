...
SOL_AMOUNT_TO_RECEIVE = float(os.environ.get("SOL_AMOUNT", "0.001"))  # Amount of SOL to receive
...
def calculate_maximum_amount_in_from_pool_state(
    ...
    try:
        ...
        print(f"Amount out: {amount_out:,} lamports ({amount_out / LAMPORTS_PER_SOL:.6f} SOL)")
        print(f"Expected input: {expected_input:,} tokens")
        print(f"Maximum with {slippage_tolerance*100}% slippage: {maximum_with_slippage:,} tokens")
...
async def main():
    ...
    try:
        ...
        async with AsyncClient(RPC_ENDPOINT) as client:
            ...
            tx_signature = await sell_exact_out(client, TOKEN_MINT_ADDRESS, SOL_AMOUNT_TO_RECEIVE, SLIPPAGE_TOLERANCE)
