...
TOKEN_AMOUNT_TO_RECEIVE = int(os.environ.get("TOKEN_AMOUNT", "1000000"))  # Amount of tokens to receive (in base units)
...
def calculate_maximum_amount_in_from_pool_state(
    ...
    try:
        ...
        if virtual_base <= amount_out:
            raise ValueError(f"Amount out ({amount_out}) cannot be >= virtual base reserves ({virtual_base})")
        ...
        print(f"Expected input: {expected_input:,} lamports ({expected_input / LAMPORTS_PER_SOL:.6f} SOL)")
        print(f"Maximum with {slippage_tolerance*100}% slippage: {maximum_with_slippage:,} lamports ({maximum_with_slippage / LAMPORTS_PER_SOL:.6f} SOL)")
...
async def buy_exact_out(
    ...
    try:
        ...
        print(f"Amount out: {amount_out} tokens")
        print(f"Maximum amount in: {maximum_amount_in} lamports ({maximum_amount_in / LAMPORTS_PER_SOL:.6f} SOL)")
...
async def main():
    ...
    try:
        ...
        async with AsyncClient(RPC_ENDPOINT) as client:
            ...
            tx_signature = await buy_exact_out(client, TOKEN_MINT_ADDRESS, TOKEN_AMOUNT_TO_RECEIVE, SLIPPAGE_TOLERANCE)
